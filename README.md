# PyTorch Implementation of NAA: Neuron Attribution-based Transferable Attack

This repository provides a PyTorch implementation of **NAA** converted from the original TensorFlow implementation of *Neuron Attribution-based Transferable Attack*.

The implementation targets the NIPS 2017 adversarial transfer setting and reproduces the original TensorFlow-style NAA pipeline using PyTorch, torchvision, and timm. The core attack mechanism is preserved: neuron attribution weights are estimated from intermediate-layer feature gradients, and adversarial examples are generated by disrupting the attribution contribution of neurons that support the original class.

Compared with the original TensorFlow implementation, this PyTorch version adapts the static-graph computation to PyTorch's dynamic computation graph while keeping the preprocessing, perturbation budget, feature-layer selection, DI, PI, momentum update, and main attack hyperparameters aligned for TensorFlow-style Inception surrogate models.
...
This implementation mainly uses pretrained models from `timm` and `torchvision`.

For TensorFlow-style Inception surrogate models, the following `timm` models are used:
...
The pretrained weights are automatically loaded by `timm.create_model(..., pretrained=True)`.

This implementation also provides basic support for torchvision models such as:
...
However, the main TensorFlow-to-PyTorch reproduction and parameter alignment are designed for TensorFlow-style Inception models.
...
The attack behavior is controlled by `--attack_method`.
...
- `DI` enables input diversity;
- `PI` enables projection-based perturbation amplification;
- momentum is used by default in the iterative update.
...
| `--attack_method` | Attack variant, e.g. `NAA`, `NAA_DI`, `NAA_PI_DI` |
| `--layer_name` | Intermediate feature layer used for neuron attribution attack |
...
The following sections preserve the implementation notes of the PyTorch NAA reproduction.
...
1. TensorFlow uses placeholders and forwards a concatenated `2B` batch of clean and adversarial images, while PyTorch forwards baseline images, integration-path images, and adversarial images separately.
...
Despite these engineering differences, the core NAA mechanism and the main attack hyperparameters are kept consistent with the TensorFlow implementation for TensorFlow-style Inception surrogate models.

---

## Citation

If you use NAA in your research, please cite the original NAA paper.

```bibtex
@inproceedings{zhang2022naa,
  title={Improving Adversarial Transferability via Neuron Attribution-Based Attacks},
  author={Zhang, Jiadong and Wu, Weibin and Huang, Jen-tse and Huang, Yizhan and Wang, Wenbo and Su, Yevgeniy and Lyu, Michael R.},
...
This PyTorch implementation follows the TensorFlow implementation style of NAA and adapts it to PyTorch/timm models.
